...
Supports batching, retry logic, rate limiting, and context passing like the
VoxDub backend but calls Gemini directly. User must provide GEMINI_API_KEY.
"""
...
# Conservative default for free tier
_RATE_LIMIT = 15
_RATE_WINDOW_S = 60.0
...
def _call_gemini(api_key: str, model: str, prompt: str, timeout: float = 120.0, *, base_url: str = "https://generativelanguage.googleapis.com/v1beta", temperature: float = 0.3) -> dict:
    """Call Gemini API and return response."""
    import requests

    url = f"{base_url.rstrip(chr(47))}/models/{model}:generateContent"
    headers = {
    ...
    try:
        ...
        raise TranslateError("Gemini API timeout") from e
    except requests.exceptions.ConnectionError as e:
        raise TranslateError("Gemini API connection error") from e
    except requests.exceptions.RequestException as e:
        raise TranslateError(f"Gemini API request failed: {e}") from e

    if resp.status_code != 200:
        error_text = resp.text[:500]
        if resp.status_code == 429:
            raise TranslateError(f"Gemini rate limit exceeded (429): {error_text}")
        elif resp.status_code >= 500:
            raise TranslateError(f"Gemini server error ({resp.status_code}): {error_text}")
        else:
            raise TranslateError(f"Gemini API error ({resp.status_code}): {error_text}")

    try:
        data = resp.json()
    except ValueError as e:
        raise TranslateError(f"Gemini returned invalid JSON: {resp.text[:500]}") from e

    if "candidates" not in data or not data["candidates"]:
        raise TranslateError(f"Gemini returned no candidates: {json.dumps(data)[:500]}")

    candidate = data["candidates"][0]
    if "content" not in candidate or "parts" not in candidate["content"]:
        raise TranslateError(f"Gemini response missing content: {json.dumps(candidate)[:500]}")

    text = candidate["content"]["parts"][0].get("text", "")
    if not text:
        raise TranslateError("Gemini returned empty text")

    return {"text": text.strip()}


def _extract_json(text: str) -> list[dict]:
    """Extract JSON array from Gemini response, handling markdown blocks."""
    text = text.strip()
    ...
    try:
        ...
        if start != -1 and end != -1 and end > start:
            try:
                ...
                raise TranslateError(f"Failed to parse JSON from Gemini: {text[:500]}") from e
        else:
            raise TranslateError(f"No JSON array found in Gemini response: {text[:500]}") from e

    if not isinstance(data, list):
        raise TranslateError(f"Gemini returned non-array JSON: {type(data)}")
...
def translate_segments(
    ...
    """Translate segments via Gemini API."""
    if not segments:
    ...
    if not api_key:
        raise TranslateError(
            "GEMINI_API_KEY not configured. Please add your Gemini API key in Settings.")
    ...
    logger.info(f"Translating {len(segments)} segments via Gemini ({model})")
    ...
    def _run_batch(index: int, batch: list[dict]) -> list[dict]:
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            try:
                ...
                result = _call_gemini(api_key, model, prompt, base_url=settings.gemini_base_url, temperature=settings.translate_temperature)
                returned = _extract_json(result["text"])
                data = {"segments": returned}
                break
            except Exception as e:
                if attempt >= _MAX_ATTEMPTS or not _is_retryable(e):
                    raise TranslateError(f"Gemini translation failed: {e}") from e
